...
class GameState:
    ...
    def generatePacmanSuccessors(self):
        ...
        return [(self.generateSuccessor(0, action), action) for action in self.getLegalPacmanActions() if
    ...
    def generateGhostSuccessors(self, index):
        ...
        if index == 0:
            ...
            raise Exception("Too many expanded nodes")
        GameState.countExpanded += 1
        return [(self.generateSuccessor(index, action), action) for action in self.getLegalActions(index) if
                action != Directions.STOP]
